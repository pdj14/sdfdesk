...
pub fn init_log(_is_async: bool, _name: &str) -> Option<flexi_logger::LoggerHandle> {
    ...
    INIT.call_once(|| {
        ...
        use flexi_logger::*;
        // Use "debug" level for debug builds, "info" for release, unless overridden by env var
        let default_level = if cfg!(debug_assertions) {
            "debug,reqwest=warn,rustls=warn"
        } else {
            "info,reqwest=warn,rustls=warn"
        };

        if let Ok(x) = Logger::try_with_env_or_str(default_level) {
            logger_holder = x
