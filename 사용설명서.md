...
- **로컬 서버**: Electron 클라이언트와 연동하여 화면 스트리밍
...
- **백엔드 (CLI)**: `target/release/sdfdesk.exe`
- **프론트엔드 (GUI)**: `electron/dist/sdfdesk-client 1.0.0.exe` (포터블 버전) 또는 `electron/dist/win-unpacked/` (압축 해제된 폴더)
...
#### 6. 화면 공유 및 제어 (로컬 서버 + Electron 클라이언트)

- **제어 기능**: 마우스 이동, 클릭(좌/우/휠), 휠 스크롤, 키보드 입력이 지원됩니다.
